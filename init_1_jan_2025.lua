...
-- Other remaps
--vim.keymap.set("i", "<leader>w", ":w", options)
...
-- Map <Leader>w to write the file
...
-- Leader+p for insert pdb
function InsertPDB()
  local trace = 'import pdb; pdb.set_trace()'
  local current_line_number = vim.api.nvim_win_get_cursor(0)[1]
  local current_line = vim.api.nvim_get_current_line()
  local lines = { current_line, trace }
  vim.api.nvim_buf_set_lines(0, current_line_number - 1, current_line_number, false, lines)
end
vim.api.nvim_set_keymap('n', '<Leader>p', ':lua InsertPDB()<CR>', { silent = true })

vim.api.nvim_set_keymap('n', '<leader>m', ":lua require'dap'.toggle_breakpoint()<CR>", { noremap = true, silent = true })
vim.api.nvim_set_keymap('n', '<leader>n', ":lua require'dap'.open()<CR>", { noremap = true, silent = true })

-- Remap for dealing with word wrap
vim.keymap.set('n', 'k', "v:count == 0 ? 'gk' : 'k'", { expr = true, silent = true })
vim.keymap.set('n', 'j', "v:count == 0 ? 'gj' : 'j'", { expr = true, silent = true })

-- Use tree-sitter to determine folding
vim.wo.foldmethod = 'expr'
vim.wo.foldexpr = 'nvim_treesitter#foldexpr()'

-- Disable manual folding
vim.wo.foldenable = false

-- Highlight TODO and FIXME keywords in comments
vim.cmd [[
